...
struct Vector {
  ...
  T const &operator[](size_t i) const noexcept { return m_data[i]; }
  ...
  T const &front() const noexcept { return *m_data; }
  ...
  T *erase(T const *it) noexcept(std::is_nothrow_move_assignable_v<T>) {
    size_t i = it - m_data;
    for (size_t j = 1 + i; j != m_size; j++) {
      m_data[j - 1] = std::move(m_data[i]);
    }
    m_size--;
    std::destroy_at(&m_data[m_size]);
  ...
  auto operator<=>(Vector const &that) noexcept {
    return std::lexicographical_compare_three_way(begin(), end(), that.begin(),
                                                  that.end());
  }
